...
DEFAULT_DOWNLOAD_FOLDER = argv[1] if len(argv) > 1 else ""
...
for i in range(1, 9):
	print("page: " + str(i))
	page = requests.get(
		'https://dati.trentino.it/dataset?tags=luoghi+e+punti+di+interesse&page=' + str(i))
		...
		for l in pageHrefs:
			...
			if "v2/content" in link and not path.exists(name + ".json"):
				try:
					time.sleep(REQUEST_DELAY)
					r = requests.get(link)
					with open(DEFAULT_DOWNLOAD_FOLDER + name + '.json', 'w') as file:
						file.write(r.text)
				except:
			...
			if "v2/geo" in link and not path.exists(name + " GEO.json"):
				try:
					time.sleep(REQUEST_DELAY)
					r = requests.get(link)
					with open(DEFAULT_DOWNLOAD_FOLDER + name + ' GEO.json', 'w') as file:
						file.write(r.text)
				except:
			...
			if "v2/classes" in link and not path.exists(name + " CLASSES.json"):
				try:
					time.sleep(REQUEST_DELAY)
					r = requests.get(link)
					with open(DEFAULT_DOWNLOAD_FOLDER + name + ' CLASSES.json', 'w') as file:
						file.write(r.text)
				except:
